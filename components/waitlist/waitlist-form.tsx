...
import { CheckCircle2, UserRound, BriefcaseBusiness, MapPin, Mail, Sparkles } from 'lucide-react';
...
export function WaitlistForm({ defaultRole, fixedRole = false, onSuccess }: WaitlistFormProps) {
  const [submittedRole, setSubmittedRole] = React.useState<'customer' | 'artisan' | null>(null);
  ...
  const onSubmit = async (data: WaitlistFormValues) => {
    ...
    const payload = {
      ...
      ...(data.role === 'artisan' && data.category ? { trade: data.category } : {}),
    };
  ...
  const inputErrorClass = (error?: unknown) =>
    cn(error ? 'border-brand-error focus:border-brand-error focus:ring-brand-error/40' : '');
      ...
      >
        <div className="space-y-4">
          ...
          <Typography variant="body">
            Sign up for early updates and access to trusted local artisans or customers in your
            area.
          </Typography>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-5" noValidate>
          <div className="grid gap-5 tablet:grid-cols-2">
            <div className="space-y-2 tablet:col-span-2">
              <label htmlFor="fullName" className="text-sm font-semibold text-brand-textPrimary">
                Full name
              </label>
              <Input
                id="fullName"
            ...
            <div className="space-y-2">
              <label htmlFor="email" className="text-sm font-semibold text-brand-textPrimary">
                Email address
              </label>
            ...
            <div className="space-y-2">
              <label htmlFor="location" className="text-sm font-semibold text-brand-textPrimary">
                Location
              </label>
              <div className="relative">
              ...
              {errors.location ? (
                <p className="text-sm text-brand-error">{errors.location.message}</p>
              ) : null}
            </div>
            ...
            <div className="space-y-2 tablet:col-span-2">
              ...
              {errors.role ? (
                <p className="text-sm text-brand-error">{errors.role.message}</p>
              ) : null}
            </div>

            {selectedRole === 'artisan' ? (
              <div className="space-y-2 tablet:col-span-2">
                <label htmlFor="category" className="text-sm font-semibold text-brand-textPrimary">
                  Service category
                </label>
                <Select
                  id="category"
                ...
                {errors.category ? (
                  <p className="text-sm text-brand-error">{errors.category.message}</p>
                ) : null}
              </div>
            ) : null}
          </div>
          ...
          >
            {isSubmitting || formStatus === 'loading' ? 'Loading...' : 'Join waitlist'}
          </Button>
        </form>
      </Card>
        ...
        <div className="flex h-full flex-col justify-between gap-8">
          <div className="space-y-4">
            ...
            <Typography variant="body" className="text-brand-accent">
              Your details are sent to the live waitlist API and saved only after the backend
              confirms the database insert.
            </Typography>
          </div>
            ...
            {formStatus === 'loading' ? (
              <div className="mt-5 rounded-2xl border border-white/20 bg-white/10 p-4 text-brand-accent">
                <div className="flex items-center gap-2 text-sm font-semibold">
                  <ProxiIcon icon="loading" className="h-4 w-4 text-white animate-spin" />
