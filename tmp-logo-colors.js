...
function paeth(a,b,c){const p=a+b-c;const pa=Math.abs(p-a);const pb=Math.abs(p-b);const pc=Math.abs(p-c);if(pa<=pb&&pa<=pc)return a; if(pb<=pc)return b; return c;}
...
for(let i=0;i<pixelData.length;i+=bytesPerPixel){
  const r=pixelData[i]; const g=pixelData[i+1]; const b=pixelData[i+2]; const a=bytesPerPixel===4?pixelData[i+3]:255;
  const key=`${r},${g},${b},${a}`;
...
const filtered = arr.filter(c=>c.opacity===255).sort((a,b)=>b.count-a.count).slice(0,120);
